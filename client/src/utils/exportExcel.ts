...
import type { SubmissionStatus, Priority, SubmissionType } from '@/constants';
...
const thinBorder = {
  top:    { style: 'thin' as const, color: { rgb: BRAND.border } },
  bottom: { style: 'thin' as const, color: { rgb: BRAND.border } },
  left:   { style: 'thin' as const, color: { rgb: BRAND.border } },
  right:  { style: 'thin' as const, color: { rgb: BRAND.border } },
};
...
export function exportToExcel(data: ExportRow[], filename: string = 'moderation-queue') {
  ...
  const headerRow = headers.map((h) => ({ v: h, t: 's' as const }));

  // ── Data rows ──
  const dataRows = data.map((row, i) => [
    { v: i + 1, t: 'n' as const },
    { v: row.title, t: 's' as const },
    { v: SUBMISSION_TYPE_LABELS[row.type] || row.type, t: 's' as const },
    { v: row.creatorName, t: 's' as const },
    { v: row.campaignTitle || '—', t: 's' as const },
    { v: PRIORITY_STYLES[row.priority]?.label || row.priority, t: 's' as const },
    { v: STATUS_STYLES[row.status]?.label || row.status, t: 's' as const },
    { v: new Date(row.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }), t: 's' as const },
    { v: row.assignedTo || 'Unassigned', t: 's' as const },
    { v: row.tags.join(', ') || '—', t: 's' as const },
  ]);
  ...
  const footerRow = [
    ...
    { v: '', t: 's' as const },
    { v: '', t: 's' as const },
    { v: `Total: ${data.length} records`, t: 's' as const },
  ];
